...
### Added
- `simulate-agent` skill: run multi-turn agent simulations with defined user personas using evaluatorq. Covers persona definition (role, tone, goals, constraints), conversation control (`max_turns`, stop tokens, simulator model selection), and output storage (orq.ai threads, `~/.evaluatorq/runs/`, auto-uploaded Experiment runs). Resources: `persona-template.md`, `simulation-loop.md` (Python + TypeScript), `redteam-mode.md` (when to switch to `evaluatorq.red_team()`). RES-732.
